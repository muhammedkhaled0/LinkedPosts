...
export default function PostCard({post,numberOfComments,getUpdatedComments}) {
  const {userData}=useContext(AuthContext);
  const {user,createdAt,body,image,comments,id}=post
  const {name,photo}=user
  const [updatedComments,setUpdatedComments]=useState(comments);
   async  function getUpdatedComments(){
    const res= await CommentsAfterDeleted(post.id);
  ...
  <div className="bg-white w-full rounded-md shadow-md h-auto py-3 px-3 my-5">
    ...
    <div className="w-full h-8 flex items-center px-3 my-3">
      ...
      <div className="w-full flex justify-between">
        <p className="ml-3 text-gray-500">{Math.ceil(Math.random()*300)}</p>
